                ...
                <div className='grid md:grid-cols-3 gap-6'>
                    {
                        poClasses.slice(0, 6).map(item => (
                            <div key={item._id} className={`shadow-xl py-5 md:py-8 px-2 md:px-3 rounded-3xl ${item.availableSeats === 0 && 'bg-red-500 bg-opacity-20'}`} data-aos="fade-right">
                                ...
                                <p className=''>Enrolled Students: <span>{item.enrolledStudent}</span></p>
                                <p className='font-semibold block md:inline md:mr-12'>Available seats: <span>{item.availableSeats}</span></p>
                                <p className='font-semibold block md:inline'>Price: $<span>{item.price}</span></p>
                            </div>
                        ))
                    }
                </div>
